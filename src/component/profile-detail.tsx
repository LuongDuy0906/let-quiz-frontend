...
export const ProfileDetail = (user: any) => {
    return (
            ...
            <div className= "flex flex-none flex-col justify-center items-center h-52 w-full mt-14 gap-8">
                {
                    !user?.user?.profile?.image ? (
                        ...
                        <img src={user?.user?.profile?.image} alt="User Avatar" className="rounded-full h-full w-fit" />
                    )
                }
                <h2 className="text-3xl font-bold">{user?.user?.profile?.username || "User name"}</h2>
            </div>
            <div className="flex flex-col justify-start items-center gap-5">
                <p className="text-3xl font-bold">
                    Bộ đề: {user?.user?.myQuizzes?.length || 0}
                </p>
                <p className="text-3xl font-bold">
                    Đánh giá: {user?.user?.averageRating?.toFixed(1) || 0}
                </p>
            </div>
